...
def gen(side, indent=4):
    ...
    ''')
    convert('''
        gradient.png -fill "rgb(31,107,194)" -colorize 100%
        -compose CopyOpacity gradient.png -composite gradient.png
    ''')

    progress(5,10,indent)
    convert(black + '''
        -fill "rgb(37,128,232)" -font {3} -pointsize {0}
        -annotate +{1}+{2} "cvr" text.png'''.format(
            side*0.55, side*0.15, side*0.75, 'Myriad-Pro-Bold-SemiCondensed'
        )
    )
    ...
    convert(blank + """
        -fill "rgb(0,92,196)" -draw "roundrectangle {0},{0} {1},{1} {2},{2}"
        base.png""".format(
            side/20, side - side/20, side/40
        )
    )
